  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (reporte.imagenes && reporte.imagenes.length > 0) {
        ...
        if (reporte.imagenes.length === 1) {
          // Una sola imagen — ocupa página completa
          try {
            const imgBuffer = await descargarImagen(reporte.imagenes[0].url);
            const maxAncho = 460;
            const maxAlto  = 340;
            ...
            doc.fillColor(ROJO_CRITICO).font("Helvetica").fontSize(10)
              .text("No se pudo cargar la imagen.", 62, y + 12);
            y += 40;
          }
        } else {
